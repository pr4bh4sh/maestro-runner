...
<hierarchy rotation="0">
  ...
  <node class="android.widget.TextView" text="Submit" error="" bounds="[0,100][1080,200]" enabled="true" displayed="true" />
	...
	elements, _ := ParsePageSource(xml)
	if len(elements) != 2 {
	...
	got := FilterBySelector(elements, flow.Selector{Text: "This is an error!"})
	if len(got) != 1 || got[0].ErrorText != "This is an error!" {
		t.Errorf("literal error text should match the field, got %d", len(got))
	}
	got = FilterBySelector(elements, flow.Selector{Text: "This is an.*"})
	if len(got) != 1 {
	...
	for _, e := range FilterBySelector(elements, flow.Selector{Text: ".*"}) {
	...
	if got := FilterBySelector(elements, flow.Selector{Text: "Submit"}); len(got) != 1 || got[0].Text != "Submit" {
